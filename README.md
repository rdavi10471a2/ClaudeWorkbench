...
- **Agent workspace (CWD & tools)** — the agent's working directory is the **watched solution's folder** (auto-derived from the host's config, so it tracks whatever `WatchedSolutionPath` points at). The watched workspace is **read-only** to the agent's built-in tools: `Read`/`Grep`/`Glob` are allowed, but `Write`/`Edit`/`Bash` are disallowed, so every mutation must go through the governed `claude-workbench` MCP (`submit_file` → stage → operator review). Native read tools are optional **per turn** (`POST /prompt { readTools: false }`) to force *all* access — reads included — through the MCP surface. The sidecar sets `strictMcpConfig: true`, so only `claude-workbench` is exposed; the machine's account/user MCP connectors (e.g. claude.ai connectors) do **not** leak into the agent.
