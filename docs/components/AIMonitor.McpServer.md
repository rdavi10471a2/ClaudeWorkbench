...
- **Residual WinMerge wording:** WinMerge is retired (review is the in-app DiffPlex surface), yet legacy naming lingers — `WinMergeCandidatePaths` in settings/status, `record_diff_decision`'s "Classify a completed WinMerge review" description, `launch_staged_diff`'s "launch WinMerge" text, and the `get_tool_manifest` Safety Note "Review uses … WinMerge review/save …". The `get_staging_guide` prose has been corrected (`edf83c8`) to say the operator reviews in-app and *not* to call `launch_staged_diff` / `record_diff_decision`, but those tool descriptions and the manifest still read WinMerge-era. The `diff-tool-available` self-check guardrail was intentionally dropped (`e83d996`) — it warned on every run once no WinMerge binary existed — and the self-check's mutation-surface message now names the operator's in-app merge-review Accept.
