...
| `WorkspaceChildProcessJob` | `WorkspaceChildProcessJob.cs` (internal, static) | Wraps the host process in a Windows Job Object (`KILL_ON_JOB_CLOSE`) so the child `dotnet BuildHost` that `MSBuildWorkspace` spawns over a pipe cannot outlive the host (or leak, holding `obj`/`bin` handles). Handle is intentionally held for process lifetime; failures are swallowed by contract. |

## How it works
`EnsureMSBuildRegistered()` runs once (guarded by a static lock + `registrationAttempted` flag). Before registering it (1) sets `MSBUILDDISABLENODEREUSE=1`, `DOTNET_CLI_DO_NOT_USE_MSBUILD_SERVER=1` and `UseSharedCompilation=false` so no persistent MSBuild worker node / dotnet MSBuild server / VBCSCompiler lingers holding handles on the watched sample's `obj`/`bin`, (2) calls `WorkspaceChildProcessJob.EnsureCurrentProcessIsJobbed()` so the spawned `BuildHost` child dies with the host, then (3) calls `MSBuildLocator.RegisterDefaults()` if not already registered — so the correct MSBuild toolset is on the resolver before any workspace is created. Each call then creates a fresh `MSBuildWorkspace`, opens the solution/project, and per project pulls a `Compilation`. Declarations are collected by walking every indexable document's syntax tree and calling `SemanticModel.GetDeclaredSymbol` on declaration nodes; references are collected in a second pass over candidate nodes (identifiers, invocations, object creations, attributes, casts, etc.), resolving each to a target symbol and matching it against a solution-wide `solutionSymbolsByIdentity` map. Symbol identity is a `StableIdentifier` hash over mapped file path + kind + display signature + line, so the same logical symbol matches across projects and across Razor mappings.
...
## Gotchas & invariants
- **MSBuild registration is once-per-process, and also installs the file-locking mitigations.** `EnsureMSBuildRegistered` (under a static lock) sets the no-node-reuse / no-build-server / no-shared-compilation env vars, jobs the process via `WorkspaceChildProcessJob` so the `BuildHost` child can't outlive it, then calls `MSBuildLocator.RegisterDefaults()` only if not already registered. Registering must happen before any Roslyn MSBuild type loads, so keep it as the first line of every public entry method. Note: `registrationAttempted` is set to `true` *before* `RegisterDefaults()` runs, so if registration throws, later calls short-circuit and proceed unregistered rather than re-throwing a clear error.
