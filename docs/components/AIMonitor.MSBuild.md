...
**Project:** `src/AIMonitor.MSBuild/AIMonitor.MSBuild.csproj` · **Depends on:** `AIMonitor.Core` (for `StableIdentifier`), Roslyn Workspaces/MSBuild (`Microsoft.CodeAnalysis.CSharp.Workspaces` 5.3.0, `Microsoft.CodeAnalysis.Workspaces.MSBuild` 5.3.0), `Microsoft.Build*` 17.11 + `Microsoft.Build.Locator` 1.11, `Microsoft.AspNetCore.Razor.Language` 6.0 · **Depended on by:** `AIMonitor.Data` (`SolutionIndexBuilder`, the main consumer), and transitively `AIMonitor.Indexing`, `AIMonitor.Cli`, `AIMonitor.McpServer`, `ClaudeWorkbench.Host`
