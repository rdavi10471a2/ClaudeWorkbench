...
sequenceDiagram
    ...
    else not accepted (e.g. rejected) BUT terminal refreshPlan supplied
        Note over SDW,PARS: !deferIndexRefresh AND refreshPlan.ChangedFilePaths > 0
        SDW->>PARS: RebuildAfterAcceptedDecision(record, refreshPlan)
        PARS-->>SDW: PostAcceptIndexRefreshResult
    end
...
- **The Blazor in-app accept path runs the terminal build itself, not through this gate.** `EngineReviewWorkflow` (`ClaudeWorkbench.Host`) calls `StagedDecisionWorkflow.Record` with `deferIndexRefresh: !terminal` and a `refreshPlan`, and deliberately **omits `terminalValidationRecords`** — since `b029a03` it runs the authoritative GATE-2 overlay build itself, *before* writing watched source, so passing them here would build twice. `ValidateTerminalPlannedOverlay` therefore short-circuits on that path; it still fires for callers that do supply the records (the MCP/CLI decision path and the unit tests). The build gate is not skipped — it just lives in the caller for the in-app path.
- **A non-accepted terminal decision can still refresh the index.** `Record` refreshes on accept, but it *also* has a second branch (`StagedDecisionWorkflow.cs:44`): when the decision is **not** accepted yet `deferIndexRefresh` is false and a `refreshPlan` with at least one changed file is supplied, it still calls `RebuildAfterAcceptedDecision`. This is the session-terminal flush — the last decision in a planned session carries the whole `refreshPlan`, so the deferred index work for earlier *accepted* siblings is applied even if the terminal file itself is rejected. (Whole-session voiding on reject is enforced a layer up, in the Host `EngineReviewWorkflow`; at this layer a lone record's reject with a plan still reconciles the index to what was actually written.)
...
- **Why the inbound-dependents guard exists (stale-key hazard, not a cascade).** The `HIGH #1` block in `PostAcceptIndexRefreshService.RebuildAfterAcceptedDecision` explains it: a project-scoped refresh of project *A* re-inserts only *A*'s own rows, so other projects' inbound reference / call-site / relationship rows that target *A*'s symbols can be left pointing at **stale target keys** (a symbol's `stable_key` can change when *A* is re-indexed). Schema v2 removed the cross-symbol `references symbols(stable_key) on delete cascade` FK, so this is a stale-key orphaning hazard, **not** a cascade-delete one. `GetInboundDependentProjectPaths` is the guard: if any other project holds inbound references into *A*, the refresh downgrades to a full solution rebuild rather than risk orphaned cross-project references. (The FK/cascade specifics live in `AIMonitor.Data` — `SolutionIndexDatabase` schema notes and `SolutionIndexStore`.)
