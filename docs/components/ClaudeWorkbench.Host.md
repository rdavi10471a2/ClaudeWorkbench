...
- **Source** (`Pages/Source/*`) — read-only browser of the watched solution, two trees in sidebar sub-tabs: **Solution** (projects → files → symbols, from the in-process AIMonitor index) and **Files** (a plain git-tracked file browser, fed by `git ls-files --cached --others --exclude-standard`, index-independent — works even before an index build). Both drive one persistent in-app **Monaco** viewer (`SourceCodeViewer`, model-swapped per file, no iframe); `.md` files render as formatted HTML via `MarkdownRenderer` with a Rendered/Raw toggle. The top toolbar adds operator **Build**/**Run** (Debug/Release, with a startup-project picker) that produce real `bin/<config>` output and **Add project** (SDK-template scaffolding). See the [Source-tab guide](../guide/source-tab.md).
...
`GitMcpTools` is the agent's **read-only** git surface: `git_status`, `git_diff`, `git_log`, `git_list_branches` auto-allow at the sidecar gate. There are deliberately **no git-write MCP tools** — commit, push, branch, and merge are operator-only actions in the Git page, which calls `GitWorkspaceService` directly (never MCP) and confirms the outward/irreversible ones. Keeping writes out of MCP means auto-approve has nothing to bypass. The Git page itself is a **read-only review** surface: the uncommitted working tree and the commit history, each click-through to a diff. The shared `Components/Shared/DiffView` renders side-by-side diffs for both the Git page and the `MergeReviewDialog` (unchanged, reused by both); `GitWorkspaceService.GetDiffContentAsync` / `GetCommitDiffContentAsync` normalize both sides to `\n` so line-ending differences don't render as spurious changes.
