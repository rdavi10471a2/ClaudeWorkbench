...
- **Batched inserts on the high-volume tables.** `documents`, `symbols`, `symbol_references`, `call_sites` and `symbol_relationships` insert through a single prepared statement reused across rows (`ExecuteMany` — prepare once, rebind parameters per row). The small per-project tables and `InsertProject` (which needs `last_insert_rowid()`) still use the per-row `Execute`. Everything is inside one transaction.
- **Counts use `count(*)`; some full scans remain by design.** `GetMonitorStatus` derives `SymbolCount`/`ReferenceCount`/`CallSiteCount`/`RelationshipCount` from `SELECT count(*)` (`store.Count*`), not by materializing rows, and `find_references_in_file` (`ListReferencesInFile`) filters by `file_path` in SQL. **Still full-read + in-memory scope:** `QueryIndex` loads the entire documents+symbols tables and scopes them in LINQ (file/folder/namespace), and staleness (next bullet) reads all documents.
...
- **Correlated caller subquery.** `ListReferences`/`ListCallSites` resolve each row's containing "caller" symbol via a correlated `select ... limit 1`. It is backed by `idx_symbols_project_file_line (project_id, file_path, start_line, end_line)`, and on the file-scoped path (`ListReferences(filePath:)`) only runs over one file's rows — but a no-filter `ListReferences()` over the whole solution still evaluates it per row.
