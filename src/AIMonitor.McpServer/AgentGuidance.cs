...
public static class AgentGuidance
{
    ...
    public static string ComposeGovernanceCard(string watchedProject)
    {
        ...
        builder.AppendLine("- DIAGRAMS: The chat also renders Mermaid. For any diagram — flowchart, sequence, class, ER, state, gitgraph — write a fenced ```mermaid code block with the diagram source and it renders inline as an SVG. PREFER a Mermaid diagram over an image or ASCII art when explaining structure, flow, or relationships (architecture, call/request flow, a class model). Fenced code blocks are also syntax-highlighted, so tag every code fence with its language (```csharp, ```sql, ```json).");
