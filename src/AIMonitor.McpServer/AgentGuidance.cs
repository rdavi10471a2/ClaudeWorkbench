...
public static class AgentGuidance
{
    ...
    private static string ComposeStagingGuide()
    {
        ...
        builder.AppendLine("5. Edit the monitor-owned Working candidate with the typed tools, and PREFER THE NARROWEST edit that fits — the same discipline as reading: for a localized change PREFER the semantic symbol/span tools (`submit_symbol`, `add_method`, `add_property`, `replace_span_in_file`), which understand structure and produce smaller, more reviewable diffs that cannot clobber unrelated content. Use `replace_text_in_file` only as a LAST RESORT, or for non-semantic files (Razor/markup, Markdown, config) where the symbol tools do not apply — not as a first choice for C#. Reserve `submit_file` (a full-file overwrite) for a NEW file or a genuine wholesale rewrite — NOT for a change that touches a few members or lines. For C# symbol edits, call `get_source_map` (selector mode) first. Submitting NO LONGER compiles the overlay — queue every planned file first.");
