...
public static class AgentGuidance
{
    ...
    private static string ComposeStagingGuide()
    {
        ...
        builder.AppendLine("5. Edit the monitor-owned Working candidate with the typed tools, and PREFER THE NARROWEST edit that fits — the same discipline as reading: for a localized change use the symbol/span tools (`submit_symbol`, `add_method`, `add_property`, `replace_span_in_file`, `replace_text_in_file`), which produce smaller, more reviewable diffs and cannot clobber unrelated content. Reserve `submit_file` (a full-file overwrite) for a NEW file or a genuine wholesale rewrite — NOT for a change that touches a few members or lines. For C# symbol edits, call `get_source_map` (selector mode) first. Submitting NO LONGER compiles the overlay — queue every planned file first.");
