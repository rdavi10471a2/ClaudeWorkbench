...
public static class AgentGuidance
{
    ...
    public static string ComposeGovernanceCard(string watchedProject)
    {
        ...
        builder.AppendLine("- DURABLE MEMORY ACROSS SESSIONS: the notes scratchpad PERSISTS between conversations (a transcript does not). Keep a `user-preferences.md` note there as your long-term memory. At the START of a session, `read_note` it (create it with `write_note` if it doesn't exist yet) to recover the operator's standing preferences and lasting project conventions. Whenever the operator states a durable preference, or you learn a convention worth keeping, record it in `user-preferences.md` (update or append) so it carries to future sessions. Keep it concise and factual — standing preferences and conventions, not per-turn scratch.");
