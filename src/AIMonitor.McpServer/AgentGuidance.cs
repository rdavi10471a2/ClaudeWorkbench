...
public static class AgentGuidance
{
    ...
    public static string ComposeGovernanceCard(string watchedProject)
    {
        ...
        builder.AppendLine("- Inspect the workspace with the claude-workbench MCP tools FIRST — they return structure and summaries, not whole files. For structural discovery prefer get_source_map, in THIS order (it is budgeted and truncates gracefully — on overflow it returns suggestedNarrowing + ready-to-run suggestedNextCalls instead of dumping): project/navigation ONLY to get the narrowing plan (at solution scale the whole project overflows) -> folder/navigation for real outlines (~5-8k tokens each) -> file/selector for stableSymbolKeys -> get_indexed_symbol / find_indexed_references for the symbol graph. get_solution_index_tree and query_solution_index are coarser raw-row tools — they are budgeted and, when a result is too large, return a compact overflow envelope (counts + highest-symbol namespaces + ready-to-run narrower calls) instead of dumping — so reach for them only when you need raw index rows; for STRUCTURE always prefer the get_source_map drill-down above. Reserve Read/Grep/Glob for when the index is stale or you need raw text the map does not carry. Do not read a whole file when an outline or selector answers the question. Verify workspace facts with a tool before stating them — never answer from memory or infer from the tool list.");
        builder.AppendLine("- BLAZOR/RAZOR references vs edits: the reference/relationship tools (find_indexed_references, find_indexed_callers, find_indexed_relationships) DO cover usages inside .razor MARKUP — they are mined from the Razor-generated C# after the build, so a razor usage resolves like any other C# reference (overloads, interfaces, renames all correct). Use them to LOCATE usages across .razor too; do NOT grep .razor for component/member usages. But they only tell you WHERE a usage is — they do NOT edit it. To CHANGE razor markup, use the TEXT tools (replace_text_in_file, replace_span_in_file, submit_file) against the monitor-owned Working candidate; the Roslyn SYMBOL tools (submit_symbol, add_method, add_property) are C#-only and CANNOT modify markup (only .cs / .razor.cs).");
        ...
        builder.AppendLine("- TWO WRITE DESTINATIONS, do not confuse them: (1) WATCHED SOURCE is read-only to you — you may only PROPOSE changes to it via the staging workflow -> operator Accept, never write it directly. (2) Your NOTES SCRATCHPAD is yours to write freely: use the `write_note` MCP tool (plus `list_notes`/`read_note`/`delete_note`) for plans, working notes, and anything to remember across the turn. It lives under the runtime folder, OUTSIDE watched source, is ungoverned (no operator gate), and can NEVER touch watched source — every note path is confined to the notes folder. Do NOT route notes through staging, and do NOT smuggle watched-source content into a note as a way to edit it.");
        builder.AppendLine("- DURABLE MEMORY ACROSS SESSIONS: the notes scratchpad PERSISTS between conversations (a transcript does not). Your long-term memory is `user-preferences.md` in that scratchpad. Its current contents are ALREADY loaded for you — appended to this card under \"Your durable preferences\" when it exists — so you never need to read it to know your standing preferences. Your job is to KEEP IT CURRENT: whenever the operator states a durable preference, or you learn a lasting convention, save it with `write_note` to `user-preferences.md` (create it the first time; otherwise read_note + rewrite the whole file with the addition merged in). Keep it concise and factual — standing preferences and conventions, not per-turn scratch. It takes effect on the next session (the card loads it at session start).");
    ...
    private static string ComposeStagingGuide()
    {
        ...
        builder.AppendLine("5. Edit at the precision you READ. You locate code with `get_source_map` at the level the change needs (project -> namespace -> folder -> file -> selector) instead of reading whole files — edit the same way. A `get_source_map` selector IS the exact coordinate for a targeted semantic edit (`submit_symbol`, `add_method`, `add_property`, `replace_span_in_file`): it lands precisely, makes a diff the operator can actually review, and cannot clobber code it never named — that precision is the payoff, not a chore. Reserve `submit_file` (a full-file overwrite) for a NEW file or a genuine wholesale rewrite, not a change touching a few members. Use `replace_text_in_file` only where a semantic edit cannot reach — Razor/markup, Markdown, config — or as a true last resort: references resolve INSIDE `.razor`, but EDITING markup is text-based (the Roslyn symbol tools are C#-only). Submitting NO LONGER compiles the overlay — queue every planned file first.");
        builder.AppendLine("6. When EVERY planned file has been submitted, call `complete_edit_plan(sessionId)` — the plan-complete event. It runs the pre-merge build ONCE over the whole set (the only build check before review, and it refuses until all planned files are submitted). If it reports errors, fix the named files, re-submit, and call it again until it is clean.");
        ...
        builder.AppendLine();
        builder.AppendLine("- NEVER dismiss a compiler diagnostic as a tooling artifact from reasoning alone. If the pre-merge build reports a diagnostic, either prove the claim with a tool or report it to the operator as unexplained and let them decide. The authoritative build also runs at the operator's accept, so a wrong dismissal is not discovered until after they have approved it.");
        ...
        builder.AppendLine("- Keep an interdependent change in ONE session: declare EVERY affected file — across ALL projects — up front in `start_monitor_session`'s `filesPlanned`, and stage them together. The plan-complete build validates only the CURRENT session's files overlaid on source, so splitting a cross-project change across separate sessions makes each build validate one half against the other's un-applied state — a false failure, or a real break that slips through. One interdependent change, one session.");
